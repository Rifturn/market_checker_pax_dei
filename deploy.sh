...
sudo add-apt-repository -y ppa:ondrej/php 2>/dev/null || echo "deb https://packages.sury.org/php/ $(lsb_release -sc) main" | sudo tee /etc/apt/sources.list.d/php.list
wget -qO - https://packages.sury.org/php/apt.gpg | sudo apt-key add - 2>/dev/null || true
sudo apt update
